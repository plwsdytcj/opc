      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          <Button size="sm" variant="outline" onClick={() => openNewIssue(newIssueDefaults())}>
            ...
            <span className="hidden sm:inline">{t("actions.newIssue")}</span>
          </Button>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? t("issues.filtersButton.count", { count: activeFilterCount }) : t("issues.filterButton")}</span>
                {activeFilterCount > 0 && (
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-auto">
                    {liveIssueIds?.has(issue.id) && (
                      <span className="inline-flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-0.5 rounded-full bg-blue-500/10">
                        ...
                        <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400 hidden sm:inline">{t("agentDetail.live")}</span>
                      </span>
                    )}
