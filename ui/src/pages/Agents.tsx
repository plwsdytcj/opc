...
export function Agents() {
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{filtered.length} {filtered.length !== 1 ? t("labels.agents") : t("labels.agent")}</p>
      )}
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] font-medium text-blue-600 dark:text-blue-400">
        {liveCount > 1 ? t("labels.liveCount", { count: liveCount }) : t("labels.live")}
      </span>
    </Link>
  );
}
